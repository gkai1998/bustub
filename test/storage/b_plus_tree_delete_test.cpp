...
TEST(BPlusTreeTests, DISABLED_DeleteTest3) {
  ...
  std::set_difference(keys.begin(), keys.end(), remove_keys.begin(), remove_keys.end(), std::back_inserter(left_keys));
  int64_t size =
      static_cast<int64_t>(std::distance(left_keys.begin(), std::find(left_keys.begin(), left_keys.end(), start_key)));
  auto current = left_keys.begin() + size;
  index_key.SetFromInteger(start_key);
  for (auto iterator = tree.Begin(index_key); iterator.isEnd() == false; ++iterator) {
    auto location = (*iterator).second;
    EXPECT_EQ(location.GetPageId(), 0);
    EXPECT_EQ(location.GetSlotNum(), *current);
    ++current;
    size = size + 1;
  }
