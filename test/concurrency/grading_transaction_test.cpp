...
TEST_F(GradingTransactionTest, IntegratedTest) {
  ...
  std::thread t1([&] {
    ...
    // 因为 txn2 delete 了一个 tuple, 假设是 tupleA, 所以 countA_val 应该等于 TEST2_SIZE - 1
    // 但如果 tupleA 在被删除前已经被计入了 txn1 aggregate 的结果之中, 测试就会失败. 也就是
    // txn1 先拿到了 tupleA 的 S lock 进行 aggregate, 等 txn1 commit 之后 txn2 才能获得
    // tupleA 的 X lock 进行 delete. 这个测试在速度快的 CPU 上可以通过, 但在速度慢的 CPU 上
    // 就会失败——事实上这样的话 txn1 会终止在下面这个 ASSERT_EQ 上而不会 commit, 导致 txn2 阻塞.
